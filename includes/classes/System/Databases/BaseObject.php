...
abstract class BaseObject
{
    ...
    public function getNextId(): int
    {
        ...
        $statement = $this->db->prepare(
            "SELECT id FROM `{$tableName}` where `{$tableName}`.id > :id LIMIT 1"
        );
        $statement->execute([':id' => $this->id]);
        $result = $statement->fetch(\PDO::FETCH_NUM);

        //If we don't have a result (end of list), get the first
        if(!$result)
        {
            $statement = $this->db->prepare(
                "SELECT id FROM `{$tableName}` where `{$tableName}`.id < :id LIMIT 1"
        ...
        if(!$result)
        {
            throw new \Exception("Query can't find {$tableName}.id.");
        }
        return $result[0];
    }
    ...
    public function getPreviousId(): int
    {
        ...
        $tableName = static::$table;
        //Get the last of everything below us
        $statement = $this->db->prepare(
            "SELECT id FROM `{$tableName}` where `{$tableName}`.id < :id ORDER BY id DESC LIMIT 1"
        );
        $statement->execute([':id' => $this->id]);
        $result = $statement->fetch(\PDO::FETCH_NUM);

        //If we don't have a result (end of list), get the last
        if(!$result)
        {
            $statement = $this->db->prepare(
                "SELECT id FROM `{$tableName}` where `{$tableName}`.id > :id ORDER BY id DESC LIMIT 1"
